// symbolic.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
public:
    ...
    static UnaryOperationExpression<T> ln(const Expression<T>& other) {
        return UnaryOperationExpression<T>(BinaryOperationExpression<T>::Type::LOG, other);
    }
};
template <typename T>
...
int main() {
    /*
    auto e1 = ConstExpression<int>(2);
    auto e2 = VariableExpression<int>("x");
    auto res = e1 + (e2 ^ e1);
    std::unordered_map<std::string, int> env;
    env["x"] = 5;
    std::cout << res.to_string() << " = " << res.eval(env) << std::endl;
    std::cout << (*res.differentiate("x")).to_string() << std::endl;
    */
    std::cout << "Here's a stub.\nI've made an Expression class and with evaluation and differentiation, but currently tests/parsing are not implemented /sadge/\nI'm going to implement it a bit later.\n";
    return 0;
}
